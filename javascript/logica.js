...
{var exp=new Array(arguments.length+1);
 exp[0] = 'step';
 for (var i=0; i<arguments.length; i++) {exp[i+1]=arguments[i]};
 return exp}
...
{if (z===y) {return x};
 ...
 if (z[0]=='forall' && z[1]===y) {return z};
 if (z[0]=='exists' && z[1]===y) {return z};
...
{if (z===y) {return x};
 if (symbolp(z)) {return z};
 if (z[0]==='forall' && (z[1]===y || amongp(z[1],x))) {return z};
 if (z[0]==='exists' && (z[1]===y || amongp(z[1],x))) {return z};
 var exp = new Array(z.length);
 for (var i=0; i<z.length; i++)
     {exp[i] = subst(x,y,z[i])}
 return exp}
...
{if (symbolp(p)) {return p};
 if (p[0] == 'not') {return makenegation(implicationsout(p[1]))};
 if (p[0] == 'and')
    {var result = seq('and');
 ...
 if (p[0] == 'or')
    {var result = seq('or');
     for (var i=1; i<p.length; i++) {result.push(implicationsout(p[i]))};
     return result};
 if (p[0] == 'reduction') {return makedisjunction(implicationsout(p[1]),makenegation(implicationsout(p[2])))};
 if (p[0] == 'implication') {return makedisjunction(makenegation(implicationsout(p[1])),implicationsout(p[2]))};
 if (p[0] == 'equivalence')
 ...
 if (p[0] == 'forall') {return seq('forall',p[1],implicationsout(p[2]))};
 if (p[0] == 'exists') {return seq('exists',p[1],implicationsout(p[2]))};
 return p}

function negationsin (p)
{if (symbolp(p)) {return p};
 if (p[0] == 'not') {return negate(p[1])};
 if (p[0] == 'and')
    {var result = seq('and');
 ...
 if (p[0] == 'or')
    {var result = seq('or');
     for (var i=1; i<p.length; i++) {result.push(negationsin(p[i]))};
     return result};
 if (p[0] == 'forall') {return makeuniversal(p[1],negationsin(p[2]))};
 if (p[0] == 'exists') {return makeexistential(p[1],negationsin(p[2]))};
 return p}

function negate (p)
{if (symbolp(p)) {return makenegation(p)};
 if (p[0] == 'not') {return negationsin(p[1])};
 if (p[0] == 'and')
    {var result = seq('or');
 ...
 if (p[0] == 'or')
    {var result = seq('and');
     for (var i=1; i<p.length; i++) {result.push(negate(p[i]))};
     return result};
 if (p[0] == 'forall') {return makeexistential(p[1],negate(p[2]))};
 if (p[0] == 'exists') {return makeuniversal(p[1],negate(p[2]))};
...
{if (varp(p))
    {if (find(p,al)) {return al};
     if (memberp(p,bl)) {return al};
     return adjoin(p,al)};
 if (symbolp(p)) {return al};
 if (p[0] == 'forall') {return freevars(p[2],al,cons(p[1],bl))};
 if (p[0] == 'exists') {return freevars(p[2],al,cons(p[1],bl))};
 for (var i=1; i<p.length; i++) {al = freevars(p[i],al,bl)};
 return al}

function disexistentialize (p,al,bl)
{if (varp(p))
    {var dum = assoc(p,bl);
     if (dum != false) {return cdr(dum)} else {return p}};
 if (symbolp(p)) {return p};
 if (p[0] == 'not') {return makenegation(disexistentialize(p[1],al,bl))};
 if (p[0] == 'and')
    {var result = seq('and');
 ...
 if (p[0] == 'or')
    {var result = seq('or');
     for (var i=1; i<p.length; i++) {result.push(disexistentialize(p[i],al,bl))};
     return result};
 if (p[0] == 'forall') {return makeuniversal(p[1],disexistentialize(p[2],cons(p[1],al),bl))};
 if (p[0] == 'exists') {return disexistentialize(p[2],al,acons(p[1],skolemterm(al),bl))};
 var exp = empty();
 for (var i=0; i<p.length; i++) {exp[exp.length] = disexistentialize(p[i],al,bl)};
 return exp}

function skolemterm(al)
{var exp = seq(newsym());
 while (!nullp(al)) {exp[exp.length] = car(al); al = cdr(al)}
 return exp}

function allsout (p)
{if (symbolp(p)) {return p};
 if (p[0] == 'not') {return makenegation(allsout(p[1]))};
 if (p[0] == 'and')
    {var result = seq('and');
...
{if (symbolp(p)) {return p};
 if (p[0] == 'and') {return distributeand(p)};
 if (p[0] == 'or') {return distributeorn(p,1)};
 return p}
...
{var exp = empty();
 ...
 for (var i=1; i<p.length; i++)
     {var arg = distribute(p[i]);
      if (symbolp(arg)) {exp[exp.length] = arg}
      else if (arg[0] == 'and') {exp = exp.concat(arg.slice(1,arg.length))}
      else exp[exp.length] = arg}
 return exp}

function distributeorn (p,n)
{if (n == p.length-1) {return distribute(p[n])};
 return disjoin(distribute(p[n]),distributeorn(p,n+1))}

function disjoin (p,q)
{if (symbolp(p))
    {if (symbolp(q)) {return makedisjunction(p,q)};
     if (q[0] == 'or') {return seq('or').concat(seq(p),q.slice(1,q.length))};
     if (q[0] == 'and')
        {var exp = seq('and');
         for (var i=1; i<q.length; i++) {exp[i] = disjoin(p,q[i])};
         return exp};
     return makedisjunction(p,q)};

 if (p[0] == 'or')
    {if (symbolp(q)) {p=p.slice(0,p.length); p.push(q); return p};
     if (q[0] == 'or') {return p.concat(q.slice(1,q.length))};
     if (q[0] == 'and')
        {var exp = seq('and');
         for (var i=1; i<q.length; i++) {exp[i] = disjoin(p,q[i])};
         return exp}
 ...
 if (p[0] == 'and')
    {if (symbolp(q))
        {var exp = seq('and');
         for (var i=1; i<p.length; i++) {exp[i] = disjoin(p[i],q)};
         return exp};
     if (q[0] == 'or')
        {var exp = seq('and');
         for (var i=1; i<p.length; i++) {exp[i] = disjoin(p[i],q)};
...
{if (symbolp(p)) {return seq(clausify(p))};
 if (p[0] == 'and')
    {var exp = empty();
     for (var i=1; i<p.length; i++)
         {exp[exp.length] = clausify(p[i])};
     return exp};
 if (p[0] == 'or') {return seq(clausify(p))};;
 return seq(clausify(p))}

function clausify (p)
{if (symbolp(p)) {return makeclause1(p)};
 if (p[0] == 'or')
    {var exp = p.slice(1,p.length);
     exp.unshift('clause');
     return uniquify(exp)};
 if (p[0] == 'clause') {return p};
...
{if (symbolp(p)) {return p};
 var exp = p[0] + '(';
 if (p.length > 1) {exp = exp + smooth(p[1])};
